...
app.get('/history', (req, res) => {
    fs.readFile(dataFilePath, 'utf8', (err, fileData) => {
        if (err) {
            ...
            res.status(500).send({ error: 'Error reading data from file' });
        } else {
            try {
                const jsonData = JSON.parse(fileData);
                console.log('Data from file:', jsonData); // Tambahkan log ini
                if (!Array.isArray(jsonData)) {
                    res.status(500).send({ error: 'Data format is not correct' });
                } else {
                    res.json(jsonData);
                }
            } catch (error) {
                console.error('Error parsing JSON data from file', error);
                res.status(500).send({ error: 'Error parsing JSON data from file' });
            }
        }
    });
});
